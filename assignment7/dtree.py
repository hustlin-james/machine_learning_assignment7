    ...
    if option == "optimized":
        ...
        for t_idx in range(len(tests_data)):
            ...
            if t[-1] == idx:
                acurracy = 1.0
                #check for ties
                num_dup = n.distribution.count(n.distribution[idx]) 
                if num_dup > 1:
                    accuracy = (1/num_dup)
                acurracy_sum += acurracy
            print("ID=%5d, predicted=%3d, true=%3d, accuracy=%4.2lf"%(t_idx,idx,t[-1],acurracy))

        print("classification accuracy=%6.4lf\n"%(acurracy_sum/float(len(tests_data))))
    else:
        trees = []
        num_times = 1
        ...
        for t_idx in range(len(tests_data)):
            t = tests_data[t_idx]
            distribution = classify_dist_prob_ary[t_idx]
            class_idx = distribution.index(max(distribution))
            acurracy = 0.0
            if t[-1] == class_idx:
                acurracy = 1.0
                #check for ties
                num_dup = distribution.count(distribution[class_idx]) 
                if num_dup > 1:
                    accuracy = (1/num_dup)
                acurracy_sum += acurracy
            print("ID=%5d, predicted=%3d, true=%3d, accuracy=%4.2lf"%(t_idx,idx,t[-1],acurracy))
